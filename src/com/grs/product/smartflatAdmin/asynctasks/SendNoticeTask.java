...
public class SendNoticeTask  extends AsyncTask<Void, Void, SmartFlatAdminError> {

	private static final String TAG = LoginTask.class.getName();
	final Context context;
	private AsyncTaskCompleteListener<Response> listener = null;
	String noticeTo,noticeSubject,noticeMessage;
	Response mNoticeStatus;
	
	public SendNoticeTask(Context ctx, AsyncTaskCompleteListener<Response> listener, String noticeTo, String noticeSubject, String noticeMessage) 
	{
		this.context = ctx;
	...
	protected SmartFlatAdminError doInBackground(Void... params) {
		...
		try 
		{
			mNoticeStatus =  smartFlatAPI.sendNotice(noticeTo, noticeSubject, noticeMessage);
		}
	...
	protected void onPostExecute(SmartFlatAdminError error) {
		...
		if(mNoticeStatus!=null)
		{
			if(listener!=null)
			{
				listener.onStoped();
				listener.onTaskComplete(mNoticeStatus);
				listener = null;
			}
		}
		else 
